...
export default function FormEditTask({ projectData }: FormCreateTaskProps) {
	...
	const dataProjectNameOption =
		data?.content &&
		data?.content?.map((item: { id: number; projectName: string }) => {
	...
	const dataFilterProjectId: any =
		data?.content &&
		data?.content?.filter((item: ProjectItem) => item.id === selectProjectName);
